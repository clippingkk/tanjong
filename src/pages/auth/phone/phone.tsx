...
type AuthApplePhoneBindProps = NativeStackScreenProps<
  RouteParamList,
  'AuthAppleBind'
>
...
function AuthApplePhoneBind(props: AuthApplePhoneBindProps) {
  ...
  useEffect(() => {
    navigation.setOptions({
      headerRight: (hprops: HeaderButtonProps) => (
        <SignupSkipButton {...hprops} idToken={props.route.params.idToken} />
      )
    })
  }, [navigation])
              ...
              {capture ? (
                <View px={6}>
                  <View justifyContent="center" width="100%" flexDir="row">
                    <Pressable onPress={onPhoneNumberConfirm}>
                      <Image
